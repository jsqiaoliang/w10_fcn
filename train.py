...
def parse_args(check=True):    
    parser = argparse.ArgumentParser()                       # 创建对象，解析器对象ArgumentParser()，可以添加参数
    parser.add_argument('--checkpoint_path', type=str)       # 增加命令行，add_argument()用来指定程序需要接受的命令参数 
    ...
    FLAGS, unparsed = parser.parse_known_args()              # 输出结果
...
upsample_factor = 8            # 上采样时候的最后采样倍数，pool3是8倍采样
...
# 调用vgg.vgg_16,vgg_16卷积池化得到的logits和end_points用于后面的上采样
...
###### 对vgg_16预训练模型中pool5的输出特征进行2x上采样
...
upsampled_logits_pool5 = tf.nn.conv2d_transpose(logits, upsample_filter_tensor_x2,
                                          ...
                                          strides=[1, 2, 2, 1],
                                          padding='SAME')
# 把pool4(1,1)卷积后的特征插入到上采样后的特征中，形成新的特征
upsampled_logits_pool4 = upsampled_logits_pool5 + aux_logits_16s


###### 对新特征upsampled_logits_pool4进行2x上采样
...
upsampled_logits_pool4 = tf.nn.conv2d_transpose(upsampled_logits_pool4, upsample_filter_tensor_x2,
                                          ...
                                          strides=[1, 2, 2, 1],
                                          padding='SAME')
# 把pool3(1,1)卷积后的特征插入到上采样后的特征中，形成新的特征
upsampled_logits = upsampled_logits_pool4 + aux_logits_8s


###### 对新特征upsampled_logits进行8x上采样
...
pred = tf.argmax(upsampled_logits, axis=3)    # 最大概率的类别
...
with tf.variable_scope("adam_vars"):
    optimizer = tf.train.AdamOptimizer(learning_rate=FLAGS.learning_rate)   # 优化器
    ...
    train_step = optimizer.apply_gradients(grads_and_vars=gradients, global_step=global_step)  # 应用梯度
